# Handoff: Earshot — "After Hours" visual direction

## Overview
The app is **Earshot** (repo `2bstone2/personal`, previously titled "Class Tracklist" in the UI — the
name change is part of this work): a browser app that listens for a few seconds every two
...
## Logo and brand
The mark, icon, and the dot system are specified in `Earshot Brand Sheet.dc.html` — open it
alongside this README. Summary:

- **Mark** — a pink arc opening left, built as a circle with only its right and top borders colored,
  rotated 45°. Stroke is ⅛ of the arc's diameter. A concentric 1.5px chartreuse "trace" arc sits
  18px larger in diameter. A pink disc at the centre, ⅓ the pink arc's diameter, counter-rotated
  −45°.
- **Wordmark** — `EARSHOT` in Archivo 800, width axis 112, uppercase, `letter-spacing: -0.03em`,
  followed by a pink period: a disc ≈28% of cap height, on the baseline, 3–5px after the T.
- **Icon** — rounded square, radius ≈23% of size, dark `#211D42` field with a hairline
  `rgba(239,237,250,0.2)` border. At 32px and below, drop the chartreuse trace and let the pink arc
  carry alone.
- **Rotation caution** — a 45° rotation makes the element's bounding box 1.41× its width. Inside a
  fixed tile, keep the arc's total size (diameter + both strokes) at or under 70% of the tile or the
  stroke escapes the corners. This is the one thing that will look broken if ignored.
- **The dot is the brand device.** The same 6–7px pink disc appears as the wordmark's period, in the
  LIVE tag, on the active nav pill, beside a running session, and beside a connected calendar.
  Rule: nothing else in the app is a circle.
- **LIVE tag** — fill `#C2447F` (a deepened pink), white 6px dot, white Space Mono 9–10px/700 at
  `letter-spacing: 0.14em`, radius 4px, padding 4–5px × 9–10px. The deepening is deliberate: white
  on palette pink `#E97BB2` measures about 2.3:1 and won't read at tag size. This tag replaces the
  pink "LISTENING" pill described under Home below — use the tag in the header and keep the event
  title in the body.
- **Favicon** — export as a PNG at 32px and 16px rather than relying on CSS; the arcs are
  reproducible in SVG if you'd rather (two arcs and a circle, no paths needed beyond that).
...
- `Earshot Brand Sheet.dc.html` — **the logo spec.** Final lockup, icon at 88/32/16, the dot system,
  header in idle and listening states, and the geometry rules.
